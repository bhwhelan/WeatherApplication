...
class MainFragment : Fragment() {
    ...
    private fun observeWeather() {
        //Show any weather/status only if location permissions are available, otherwise ask user
        with(requireContext()) {
            if (hasLocationPermissions() && hasInternet()) {
                with(weatherViewModel) {
                    viewModelScope.launch {
                        //Retreive latest stored weather (and/or wait until network retrieval of new)
    ...
    //Check here for pre-requisites needed to get weather (location permissions, network on, etc.)
